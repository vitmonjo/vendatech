...
export class AlertService {
  ...
  private alertedProducts = new Set<string>(); // Controla produtos já alertados
  ...
  startPriceCheck(): void {
    this.checkInterval = setInterval(() => {
      this.checkPrices();
    }, 30000); // Checa a cada 30 segundos (otimizado)
  }
  ...
  checkPrices(): void {
    this.productService.getProducts().subscribe((response) => {
      ...
      const affordableProducts = response.data.products.filter((product: Product) => product.price < 2000);
      
      // Só alerta produtos que ainda não foram alertados recentemente
      affordableProducts.forEach((product: Product) => {
  ...
  resetProductAlert(productId: string): void {
